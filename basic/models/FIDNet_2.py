...
class SpecFreBlock(nn.Module):
    def __init__(self, C_in, channels, stride,weight_factor=0.):
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.att1 = SelfAttention1D(dim=channels, num_heads=1, bias=False)
        self.att2 = SelfAttention1D(dim=channels, num_heads=1, bias=False)
    ...
    def forward(self, x):
        ...
        x_seq_1 = self.act_1(self.att1(x_g))
...
class SimVP(nn.Module):
    def __init__(self,
        ...
        T, C = 31,1
        N_S = 2
        self.spa_up = nn.Conv2d(C,hid_S,3,1,1)
        self.spec_up = nn.Conv2d(C,hid_S,3,1,1)
        # self.enc = Encoder(hid_S, hid_S, N_S)
        self.enc_spa_fre = Encoder_spa_fre(T, T, N_S)
    ...
    def forward(self, x_raw):
        ...
        x_rec = x.view(B,T,_C, H, W)
        x_rec = x_rec.transpose(1,2)
        x_rec = x_rec.reshape(B * _C,T, H, W)
        
        embed_spec_fre, feat_spec_fre = self.enc_spec_fre(x_rec)
        
        embed_spa_fre, feat_spa_fre = self.enc_spa_fre(x_rec)
        ...
        embed_spa_fre = embed_spa_fre.view(B,T,_C, H, W)
        embed_spa_fre = embed_spa_fre.transpose(1,2)
        embed_spa_fre = embed_spa_fre.reshape(B * T, _C, H, W)
        
        embed_spa_fre = self.spa_up(embed_spa_fre)
        embed_spec_fre = self.spec_up(embed_spec_fre)
        
        embed = self.fusion(embed_spec_fre,embed_spa_fre)
...
class FIDNet_2(nn.Module):
    def __init__(self,
                 hid_S,
                 hid_T,
                 opt = None):
        super(FIDNet_2, self).__init__()
        self.fourierNet = SimVP(hid_S = hid_S,
                                hid_T = hid_T)
