...
class PatchUnembed(nn.Module):
    """ Patch Unembedding Module
        Reverses the patch embedding operation from PatchEmbed.
    """
    def __init__(self, img_size=224, patch_size=16, kernel_size=1, in_chans=1, embed_dim=64):
        super().__init__()
        ...
        # Inverted operation from PatchEmbed:
        # We use transposed convolution to achieve upsampling
        self.unproj = nn.ConvTranspose3d(
            in_channels=embed_dim,  # <-- Modified here
            out_channels=1,  # Assuming original input channel
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, kernel_size=1, in_chans=1, embed_dim=256):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        num_patches = (img_size[1] // patch_size[1]) * (img_size[0] // patch_size[0])
...
class VideoMamba(nn.Module):
    ...
    def forward_features(self, x, mask=None, use_image=False):
        ...
        x_vis = self.norm2(x_vis)  # b seq_len c
        B1,N1,C = x_vis.shape
        x_vis = x_vis.reshape(B1//T, C, B1//B, N1//H, N1//W)
        x_vis = self.patch_unembed(x_vis)
