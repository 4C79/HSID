...
class FreBlock(nn.Module):
    def __init__(self, C_in, channels, stride,epsilon_init=0.5):
        ...
        # self.epsilon = torch.nn.Parameter(torch.tensor(epsilon_init), requires_grad=True)
    ...
    def forward(self, x):
        ...
        # out = out - torch.mean(out, dim=1, keepdim=True) + torch.mean(x, dim=1, keepdim=True)
        # out = torch.clip(out, float(x.min()-self.epsilon), float(x.max()+self.epsilon))
...
class SpecFreBlock(nn.Module):
    def __init__(self, C_in, channels, stride,weight_factor=0.1,epsilon_init=0.5):
    ...
    def forward(self, x):
        ...
        x_seq_1 = self.act_1(self.conv_1(x_g))
        x_seq_2 = self.act_2(self.conv_2(x_g))
        ...
        amp_adj = msF_amp * x_seq_1 + msF_amp
        pha_adj = msF_pha * x_seq_2 + msF_pha
        ...
        # # 进行一维傅里叶变换
        # fourier_transform = torch.fft.rfft(signal)

        # # 提取幅度和相位信息
        # amplitude = torch.abs(fourier_transform)
        # phase = torch.angle(fourier_transform)

        # # 构造新的傅里叶变换（使用不同的幅度和相位）
        # new_fourier_transform = amplitude * torch.exp(1j * phase)

        # # 进行实数反傅里叶变换
        # reconstructed_signal = torch.fft.irfft(new_fourier_transform, n=signal.size(0))
...
class SimVP(nn.Module):
    def __init__(self,
        ...
        self.enc_spa_fre = Encoder_spa_fre(hid_S, hid_S, N_S)
        self.enc_spec_fre = Encoder_spec_fre(T, T, N_S)
        self.fusion_1 = Fusion(dim=hid_S)
        self.fusion_2 = Fusion(dim=hid_S)
    ...
    def forward(self, x_raw):
        ...
        x_spec = x.view(B,T,_C, H, W)
        x_spec = x_spec.transpose(1,2)
        x_spec = x_spec.reshape(B * _C,T, H, W)
        ...
        embed_spec_fre, feat_spec_fre = self.enc_spec_fre(x_spec)
        
        embed_spec_fre = embed_spec_fre.view(B,T,_C, H, W)
        embed_spec_fre = embed_spec_fre.transpose(1,2)
        embed_spec_fre = embed_spec_fre.reshape(B * T, _C, H, W)
        
        embed_spa_fre, feat_spa_fre = self.enc_spa_fre(x)

        embed_fre = self.fusion_1(embed_spa_fre,embed_spec_fre)
        embed = self.fusion_2(embed,embed_fre)
...
# class FIDNet_1(nn.Module):
#     def __init__(self,
#                  hid_S,
#                  hid_T,
#                  opt = None):
#         super(FIDNet_1, self).__init__()
