...
class SAHABs(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        """
        x = x.permute(0, 2, 3, 1)
        mask = mask.squeeze(2,3)             # 这里如果在光谱维度做，改为 mask.squeeze(2,3)
...
class Sparsity_Estimator(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sparse:
            error_map = out[:,-1:,:,:] # 这里是使用了out最后一个通道的元素作为error_map
            return out[:,:-1], error_map
        return out

class CST(nn.Module):
    def __init__(self, dim, stage, num_blocks, sparse):
    ...
    def forward(self, x, mask=None):
        ...
        # else:
        #     fea = self.fe(x)
        mask = torch.randn((b,c,c,1,1)).cuda()    #用这个就行 mask = torch.randn((b,c,1,1)).cuda() ,2 值
