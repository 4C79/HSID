...
class MambaDNet(nn.Module):
    def __init__(self,
        ...
        self.reduce_chan_level3 = nn.Conv2d(int(dim), int(dim), kernel_size=1, bias=bias)
        ...
        self.reduce_chan_level2 = nn.Conv2d(int(dim), int(dim), kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img):
        ...
        inp_enc_level3 = self.down2_3(out_enc_level2, H, W)  # b, hw//16, 4c
        out_enc_level3 = inp_enc_level3
        for layer in self.encoder_level3:
            out_enc_level3 = layer(out_enc_level3, [H, W])

        inp_enc_level4 = self.down3_4(out_enc_level3, H, W)  # b, hw//64, 8c
        latent = inp_enc_level4
        for layer in self.latent:
            latent = layer(latent, [H, W])

        inp_dec_level3 = self.up4_3(latent, H, W)  # b, hw//16, 4c
        ...
        for layer in self.decoder_level3:
            out_dec_level3 = layer(out_dec_level3, [H, W])

        inp_dec_level2 = self.up3_2(out_dec_level3, H ,W)  # b, hw//4, 2c
        inp_dec_level2 =inp_dec_level2 + out_enc_level2
        inp_dec_level2 = rearrange(inp_dec_level2, "b (h w) c -> b c h w", h=H, w=W).contiguous()
        ...
        for layer in self.decoder_level2:
            out_dec_level2 = layer(out_dec_level2, [H, W])

        inp_dec_level1 = self.up2_1(out_dec_level2, H, W)  # b, hw, c
...
if __name__ == '__main__':
    ...
    # print(model)
    x = torch.randn((1, 3, height, width)).to('cuda:0')
    print(x.shape)
    y = model(x)
    print(y.shape)