...
class SSMA(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,drop_path=0.0,
        ...
        self.spectral_attn = SS2D(d_model=dim, d_state=16,expand=2,dropout=0)
    ...
    def forward(self, HSI):
        ...
        for pos in range(num):
            ...
            if (last == 0 or pos< num -1):
                denoised_HSI[:, pos * 31:(pos+1)*31, :, :] = x
            else:
                denoised_HSI[:, B-last:B, :, :] = x[:,31-last:31,:,:]
