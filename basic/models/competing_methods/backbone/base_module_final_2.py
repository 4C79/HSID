...
class Decoder(nn.Module):
    def __init__(self, hidden_dim=14,cor=1,dr=1,atten=1,
        ...
        self.conv_out_4 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1, bias=True)
        # self.conv_out_5 = nn.Conv2d(hidden_dim, hidden_dim, 3, 1, 1, bias=True)
        self.conv_out_6 = nn.Conv2d(hidden_dim, 1, 3, 1, 1, bias=True)
    ...
    def hidden2out_forward(self, x_fea, base):
        out = self.lrelu(self.conv_out_1(x_fea))
        # out = self.lrelu(self.conv_out_2(out))
        out = self.conv_out_3(out)
        return out + base
    
    def hidden2out_back(self, x_fea, base):
        out = self.lrelu(self.conv_out_4(x_fea))
        # out = self.lrelu(self.conv_out_5(out))
        out = self.conv_out_6(out)
        return out + base
    ...
    def forward(self, x, encoder_out):
        ...
        # for i in range(0 , b):
            ...
            output = self.hidden2out_forward(hidden_state, base[:,i])
            input = self.out2inp(output)

            outputs_forward.append(output)
        outputs_first = torch.stack(outputs_forward, dim=1).squeeze(dim=2)
        ...
        if back:    
            ...
            for i in range(0 , b):
                ...
                output = self.hidden2out_back(hidden_state, base[:,i])
                outputs_backward.append(output)
        if not back:
            outputs = outputs[::-1]
        outputs = torch.stack(outputs_backward, dim=1).squeeze(dim=2)

        # Todo : 返回有问题，暂修改为outputs
        return outputs,outputs_first
        # return outputs
