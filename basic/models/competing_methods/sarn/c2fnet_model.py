...
from hsi_pipeline.models.c2f import C2FNet
...
class C2FNetModel(BaseModel):
    ...
    def set_input(self, input, cur_iters=None):
        ...
        self.img_NO = input['NO'].cuda()
        self.img_GT = input['GT'].cuda()
        ...
        return self.img_NO,self.img_GT
            ...
            if self.opt.stage == 1:
                ...
                if self.opt.coarse ==1:
                    if self.opt.align == 1:
                        ...
                        self.img_DN,self.coarse = self.netG(self.img_NO)

                else: self.img_DN,self.F, self.R = self.netG(self.img_NO)
        else:
            if self.opt.align == 1:
               self.img_DN = self.netG(self.img_NO,self.img_GT_F,self.img_GT_R)
            else:self.img_DN = self.netG(self.img_NO)
            return self.img_DN
            ...
            if self.opt.stage == 1:
                ...
                if self.opt.coarse ==1:
                    ...
                    if self.opt.align == 1:
                        ...
                        self.loss_Pix = (0.1*coarse_loss + 0.1*(F_loss + R_loss) + refine_loss)
                    else:
                        coarse_loss = self.criterionL1(self.coarse, self.img_GT) * self.opt.lambda_pix
                        refine_loss = self.criterionL1(self.img_DN, self.img_GT) * self.opt.lambda_pix
                        self.loss_Pix = (0.1*coarse_loss + refine_loss)
                else:
                    F_loss = self.criterionL1(self.F, self.img_GT_F) * self.opt.lambda_pix
                    R_loss = self.criterionL1(self.R, self.img_GT_R) * self.opt.lambda_pix
                    refine_loss = self.criterionL1(self.img_DN, self.img_GT) * self.opt.lambda_pix
                    self.loss_Pix = (0.1*(F_loss + R_loss) + refine_loss)
