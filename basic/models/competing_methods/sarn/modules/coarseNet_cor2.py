...
class COARSENet(nn.Module):
    ...
    def __init__(
        ...
        self.key.append(nn.Conv2d(self.in_channels, channels, 3, 1, 1))

        # self.key.append(nn.LeakyReLU(0.1))
    ...
    def recu_unit(self, input, hidden_f=None):
        ...
        if self.opt.coarse:
            ...
            C_t = self.key(hidden_low)

            if self.opt.align == 1:
                F_t, R_t = self.spectral_residual(hidden_low)
                return hidden_low, F_t, C_t, R_t

            else: return hidden_low, C_t
    ...
    def forward(self, input, pos):
        ...
        if self.opt.coarse == 1:
            if self.opt.align == 1:
                ...
                self.hidden_f,self.C_t =  self.recu_unit(input,hidden_f=self.hidden_f)
                return self.C_t
        else:
