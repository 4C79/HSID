...
def select_patches_perturbed_topk(flatten_scores,
  ...
  batch_topk_fn = jax.vmap(
      functools.partial(perturbed_sorted_topk_indicators,
                        num_samples=num_samples,
                        sigma=sigma,
                        k=k))

  rng_keys = jax.random.split(random.PRNGKey(batch_size))
...
#     k = 2
#     x = jnp.array([[1,2,3],[2,3,4]])
#     sigma = 0.05
#     # print(random.PRNGKey(10))
#     top_k_indicators = select_patches_perturbed_topk(x,sigma,k=k)
#     print(top_k_indicators.shape)
