...
class MemNet(nn.Module):
    def __init__(self, in_channels, channels, out_channels, num_memblock, num_resblock):
        super(MemNet, self).__init__()
        self.pos = in_channels//2
        self.feature_extractor = BNReLUConv(in_channels, channels)
        self.reconstructor = BNReLUConv(channels, out_channels)
    ...
    def forward(self, x):
        residual = x[:,self.pos:self.pos+1,:,:]
