...
def noise_est(y):
    ...
    n = y.shape[0]
    noise_map = np.empty(y.shape, "float32")
    for i in range(n):
        single_img = np.squeeze(y[i, :, :, :]).permute([1, 2, 0])
        sigma = np.array(
            estimate_sigma(single_img.detach().cpu(), channel_axis=2, average_sigmas=False)).astype(
            "float32")
        one = np.ones(single_img.shape)
        noise_map[i, :, :, :] = (one * sigma).transpose([2, 0, 1])

    noise_map = torch.Tensor(noise_map).cuda()
    return noise_map
