...
import time
from utils.options import  reverse_permut_np,zigzag_path
...
class build_model(object):
    ...
    def __step(self, train, inputs, targets):  
        ...
        if self.opt['arch'] == 's2s':
            outputs,outputs_1 = self.net(inputs)
            loss = self.criterion(outputs[...], targets)*1.0 + self.criterion(outputs_1[...], targets)*0.01
        elif 'mmd' in self.opt['arch']:
            outputs = self.net(inputs,self.zig_path,self.zig_back)
